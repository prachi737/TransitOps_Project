...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    try {
      await query(
        `INSERT INTO vehicles (reg_number, name, type, region, max_load, odometer, acquisition_cost, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'Available')`,
        [
          regNumber,
          name,
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    await query(
      `UPDATE vehicles SET name = ?, type = ?, region = ?, max_load = ?, odometer = ?, acquisition_cost = ? WHERE id = ?`,
      [
        name.trim(),
        type.trim(),
        (region || "").trim() || null,
        parsedMaxLoad,
        Number.isFinite(parsedOdometer) ? parsedOdometer : vehicle.odometer,
        Number.isFinite(parsedAcquisitionCost) ? parsedAcquisitionCost : vehicle.acquisition_cost,
        id,
      ]
    );
  ...
  asyncRoute(async (req, res) => {
    ...
    try {
      await withTransaction(async (conn) => {
        const [[vehicle]] = await conn.query("SELECT * FROM vehicles WHERE id = ? FOR UPDATE", [id]);
        if (!vehicle) {
          throw { businessError: "Vehicle not found." };
        }
        if (vehicle.status === "On Trip" || vehicle.status === "In Shop") {
          throw {
            businessError: `Cannot retire ${vehicle.reg_number} while it is ${vehicle.status.toLowerCase()}.`,
          };
        }

        newStatus = vehicle.status === "Retired" ? "Available" : "Retired";
        await conn.query("UPDATE vehicles SET status = ? WHERE id = ?", [newStatus, id]);
      });
